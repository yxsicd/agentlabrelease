...
class MultiRepoModelConstructionTest(unittest.TestCase):
    ...
    def test_workflow_keeps_gateway_secret_on_trusted_main_step(self):
        ...
        self.assertIn("--semantic-packet", workflow)
        self.assertIn("--semantic-decision", workflow)
    ...
    def test_review_and_campaign_workflows_preserve_trusted_boundaries(self):
        ...
        self.assertIn("scripts/multi-repo-calibration-bundle.py run", review)
        self.assertNotIn("source/operator-fixture", review)
        self.assertNotIn("operator-fixture", campaign)
        self.assertIn("scripts/prepare-multi-repo-analysis-sources.py", campaign)
        self.assertIn("case/blind-cut/evaluator/oracle.mjs", campaign)
        self.assertNotIn("model_a:", campaign)
        self.assertNotIn("model_b:", campaign)
        self.assertIn("participant_profiles_json:", campaign)
        self.assertIn("scripts/participant-experiment-plan.py create", campaign)
        self.assertIn("participant-experiment-plan.json", campaign)
        self.assertIn('--required-trials "$TRIALS"', campaign)
