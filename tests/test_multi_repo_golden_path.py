...
class MultiRepoGoldenPathTest(unittest.TestCase):
    ...
    def test_release_ci_runs_and_retains_the_integrated_chain(self):
        ...
        self.assertIn('"calibration-authoring"', source)
        self.assertIn('"calibration-authoring-validate"', source)
        self.assertIn('"calibrationAuthoringReceiptSha256"', source)
        self.assertIn('"independentEvaluatorAuthoringProtocolQualified": True', source)
        self.assertIn('"--authoring-receipt", authoring_receipt', source)
        self.assertIn('"--authoring-root", authoring', source)
