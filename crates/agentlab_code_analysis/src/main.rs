...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let receipt = json!({"schema":"agentlab.ast_analysis.v1","sourceRevision":revision,
        "analyzer":"agentlab-code-analysis@0.1.0","analyzerDigest":agentlab_code_analysis::ANALYZER_DIGEST,
        "parser":"tree-sitter@0.27.0","grammar":agentlab_code_analysis::GRAMMAR,"grammarDigest":agentlab_code_analysis::GRAMMAR_DIGEST,
        "files":files,"filesWithSyntaxErrors":errors,"erroredPaths":errored_paths,"rows":rows.len(),"kinds":kinds,
        "sha256":digest(&bytes),"coverage":"CST-derived syntax and expression facts; no type, call-target or dataflow resolution","syntaxClean":errors==0,"fullArkTSQualified":false});
    fs::write(
