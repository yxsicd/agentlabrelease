...
fn contract(scenario: &str) -> Value {
    let (id, scope, phenomenon, cause, change, fact, skill, body, expected, qualification) = match scenario {
        ...
        "cache-durability" => ("cache-write-settlement-chain", "actual-cache-persistence-seam", "Cache callers can resolve before flush and lose write failures", "Durability promises are not propagated through persistence helper and model", "Await flush through the selected chain; retain fresh network data on persistence failure and use offline fallback only on network failure", "lesson-cache-durability", "skill-cache-durability-method", "Execute submitted persistence manager, cache helper, service and model with controlled flush. Check settlement order, neighboring records, synchronous put failure and network fallback. Require lost-flush and lost-model-await negatives to fail turn2; lost-model-await must pass turn1. Full compilation and SessionFS/device are independently qualified.", json!({"baseline":false,"reference":true,"lost-flush":false,"lost-model-await":false}), "actual four-module path with controlled Preferences/network; not UI or SessionFS"),
