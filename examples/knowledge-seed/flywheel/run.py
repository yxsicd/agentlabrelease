...
def main():
 ...
 required={(consumer,leaf) for consumer in [SPECS[-1][2][1],SPECS[3][2][2]] for leaf in [SPECS[-1][2][0],SPECS[3][2][0]]}
 observed={(row[0]['value'],row[3]['value']) for row in graph_result['rows']}
 if not required.issubset(observed):raise RuntimeError('Missing established package export paths; preserve analysis result')
 graph=dict(id='analysis-common-barrel',kind='analysis',sourceRevision=PIN,code=graph_sql,request=graph_request,result=graph_result,interpretation=str(len(graph_result['rows']))+' package-export path candidates, not resolved imported-symbol calls. Consumer -> common Index -> util index -> exported leaf.')
 updates=[]
