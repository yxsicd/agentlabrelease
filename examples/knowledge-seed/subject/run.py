...
def main():
 ...
 try:
  ...
  if a.timeout_feedback_only:
   ...
   if a.timeout_verification_cadence:
    feedback+=(' Execution-cadence guidance: after the first minimal source change, run the narrowest existing validation you can access before broadening the edit. '
               'Use that observed validation result to decide whether another source change is needed. Do not expand the frozen edit boundary.')
   ...
   summary['timeoutFeedbackOnly']=dict(enabled=True,inputPhase='turn-1',repairPhase='timeout-feedback-repair',sameNativeSession=True,sameTaskDemand=True,taskDemandIndex=(resume_demand_index if resume_manifest else 0),oracleStage=(resume_oracle_stage if resume_manifest else 1),providerReasoningEffort='default',piThinkingMode='off',verificationCadenceGuidance=bool(a.timeout_verification_cadence),gatewayRequests=repair_requests,repairLatencyMs=repair_duration,inputSessionSha256=input_session_sha,outputSessionSha256=output_session_sha,sessionAdvanced=True)
