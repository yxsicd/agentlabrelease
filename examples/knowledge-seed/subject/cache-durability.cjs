...
const ts=require(process.env.AGENTLAB_ORACLE_TYPESCRIPT),root=process.argv[2];
...
(async()=>{
 ...
 async function trial(entry,rejectFlush=false){
  const events=[],logs=[],data={currentPage:1,pageSize:30,data:{sampleCategories:[]}},store=new Map();let settle;
  ...
  const preferences={putSync:(key,value)=>{events.push('put');store.set(key,value);},hasSync:key=>store.has(key),getSync:key=>store.get(key),flush:()=>{events.push('flush-started');return flush;}};
  const logger={error:(...args)=>logs.push(args),info:()=>{}};
  const loaded={};const common={Logger:logger,ImageKnifeUtil:{preloadImg:()=>{}},TriggerEnums:{SAMPLE_PAGE:'page'},HmosRequest:{getInstance:()=>({request:async()=>{events.push('network');return data;}})}};
  ...
  return {entry,rejectFlush,pendingBeforeFlush,settled,outcome,returnedFreshData:value===data,events,logs,stored:JSON.parse(store.get('page'))};
 }
 const trials=[];for(const entry of ['helper','service','model'])trials.push(await trial(entry));trials.push(await trial('helper',true));trials.push(await trial('model',true));
 const checks={sourceSyntaxParsed:Object.values(analyses).every(a=>!a.syntaxHasErrors),helperWaitsForFlush:trials[0].pendingBeforeFlush,serviceWaitsForFlush:trials[1].pendingBeforeFlush,modelWaitsForFlush:trials[2].pendingBeforeFlush,helperPropagatesFlushFailure:trials[3].outcome==='rejected',modelRetainsFreshDataAfterFlushFailure:trials[4].pendingBeforeFlush&&trials[4].returnedFreshData&&trials[4].logs.length>0,recordAndFreshDataPreserved:trials.slice(0,3).every(t=>t.stored['1_30'].currentPage===1)&&trials[2].returnedFreshData};
 console.log(JSON.stringify({schema:'agentlab.cache_durability_probe.v1',sourceRevision:cp.spawnSync('git',['-C',root,'rev-parse','HEAD'],{encoding:'utf8'}).stdout.trim()||null,checks,pass:Object.values(checks).every(Boolean),trials,sourceFiles:Object.entries(files).map(([key,file])=>({path:file,sha256:crypto.createHash('sha256').update(sources[key]).digest('hex'),source:sources[key],analysis:analyses[key]})),referenceTransformsApplied:false,actualModulesExecuted:4,qualification:{candidateOnly:true,phoneBuild:false,pageUI:false,actualAgent:false}}));
