"""Capture staged subject evidence and feedback in the existing knowledge tables."""
...
def main():
 ...
 for path in sorted(a.evidence.rglob('*')):
  ...
  key=pre+'file-'+hashlib.sha256(name.encode()).hexdigest()[:20];facts.append(dict(id=key,kind='assessment-file',path=name,fileRole=role,sha256=digest,byteCount=len(raw),chunkIds=ids,**lineage));files[key]=(raw,ids)
  if name.endswith('events.jsonl'):
   for number,line in enumerate(raw.splitlines(),1):
    if not line.strip():continue
    try:event=json.loads(line)
    except ValueError:continue
    facts.append(dict(id=pre+'native-'+hashlib.sha256(name.encode()).hexdigest()[:12]+'-'+str(number),kind='native-event-index',eventType=event.get('type'),toolName=event.get('toolName'),toolCallId=event.get('toolCallId'),reportedIsError=event.get('isError'),participant=name.split('/')[0],nativeFileId=key,lineNumber=number,observationAuthority='participant-adapter',**lineage))
  if '/gateway/' in name and name.endswith('.request.json'):
   wire=json.loads(raw)
   facts.append(dict(id=pre+'request-'+hashlib.sha256(name.encode()).hexdigest()[:16],kind='gateway-request-index',participant=name.split('/')[0],model=wire.get('model'),messageCount=len(wire.get('messages',[])),toolCount=len(wire.get('tools',[])),wireFileId=key,**lineage))
   for number,tool in enumerate(wire.get('tools',[])):
    function=tool.get('function',{})
    facts.append(dict(id=pre+'tool-definition-'+hashlib.sha256(name.encode()).hexdigest()[:16]+'-'+str(number),kind='gateway-tool-definition',toolName=function.get('name'),inputSchema=function.get('parameters'),wireFileId=key,jsonPointer='/tools/'+str(number),**lineage))
   for number,message in enumerate(wire.get('messages',[])):
    facts.append(dict(id=pre+'message-'+hashlib.sha256(name.encode()).hexdigest()[:16]+'-'+str(number),kind='gateway-context-message-index',role=message.get('role'),toolCallId=message.get('tool_call_id'),toolCallCount=len(message.get('tool_calls',[])),wireFileId=key,jsonPointer='/messages/'+str(number),**lineage))
 ...
 durable=store.read(s,repo,rev,PREFIX+'program_facts')
 for raw,ids in files.values():assert raw==b''.join(base64.b64decode(durable[k]['value']) for k in ids)
 ...
 result=dict(ok=True,revision=rev,fileCount=len(files),allFilesExact=True,stableExport=True,reimportUnchanged=True,subjectTaskSucceeded=summary['subjectTaskSucceeded']);(a.root/'receipt.json').write_text(json.dumps(result,indent=2)+'\n');print(json.dumps(result))
